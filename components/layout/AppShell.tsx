...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'var(--bg-0)' }}>
      {/* Sidebar */}
      <div
      ...
      {/* Main */}
      <div className="flex-1 overflow-auto p-6" style={{ maxHeight: '100vh' }}>
        {!role.boards.includes(currentBoard) ? (
          <div className="text-center py-20">
            <Shield size={40} style={{ color: 'var(--mut)', margin: '0 auto 16px' }} />
            <h3 className="text-lg font-bold mb-2">Access restricted</h3>
            <p className="text-sm" style={{ color: 'var(--mut)' }}>
              Your role ({role.label}) doesn&apos;t have access to this board.
            </p>
          </div>
        ) : children}
      </div>
